...
def write2sql(df):
    ...
    try:   
        ...
        df.to_sql('test_table_2', con = engine, if_exists = 'append', index = False)
...
def translate_waymo2dataframe(FILE_PATH):
    ...
    scene_meta_dict['scene_id'] = str(scene_id)
    scene_meta_dict['camera_object_counts'] = str(stats.camera_object_counts).replace('\n',',')
    scene_meta_dict['laser_object_counts'] = str(stats.laser_object_counts).replace('\n',',')
    scene_meta_dict['time_of_day'] = str(stats.time_of_day)
    scene_meta_dict['location'] = str(stats.location)
    scene_meta_dict['weather'] = str(stats.weather)


    
    df_meta = pd.DataFrame.from_dict([scene_meta_dict])
